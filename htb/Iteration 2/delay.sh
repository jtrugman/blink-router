...
echo "Link eth0 & ifb0"
echo "------------------------------------------"
tc qdisc add dev br-lan root handle 1: prio # sets up token bucket
echo "added br-lan htb default"
tc class add dev br-lan parent 1:3 parent 1:3 handle 30: \
netem delay 100000ms
echo "------------------------------------------"
tc qdisc add dev ifb0 root handle 1: prio
echo "added ifb htb default"
tc class add dev ifb0 parent 1:3 handle 30: \
netem delay 100000ms
echo "added first ifb rate"
echo "------------------------------------------"
tc filter add dev br-lan protocol ip parent 1:0 prio 3 u32 \
    match ip dst 192.168.8.177/32 flowid 1:3 # replace IP address and flow id with corresponding from above
